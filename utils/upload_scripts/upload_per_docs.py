...
for file_name, file_path in files_to_upload:
    ...
    try:
        # Following documentation from https://docs.sevenbridges.com/reference/upload-a-file-to-the-server
        my_file = File(api=api, file_name=file_name)
        my_file.upload(project=project.id, path=file_path, file_name=file_name)
