...
perform_enrichment_gsea <- function(diffexpr_res,
                                    pathways,
                                    minGSSize = 10,
                                    maxGSSize = 500,
                                    prefix,
  ...
  path_res <- path_res %>%
    mutate(comparison = prefix)
  write_tsv(x = path_res %>% as.data.frame(),
            file = file.path(results_dir, paste0(prefix, "_gsea.tsv")))
  ...
  if (nrow(top10_output) > 0) {
    # barplot
    pdf(
      file = file.path(plots_dir, paste0(prefix, "_gsea_barplot.pdf")),
      width = 8,
      height = 8
    )
  ...
  if (nrow(path_res) > 0) {
    # dotplot
    pdf(
      file = file.path(plots_dir, paste0(prefix, '_gsea_dotplot.pdf')),
      width = 10,
      height = 10
    )
    ...
    # network plot
    pdf(
      file = file.path(plots_dir, paste0(prefix, '_gsea_cnet.pdf')),
      width = 12,
      height = 10
    )
    p <- cnetplot(path_res,
